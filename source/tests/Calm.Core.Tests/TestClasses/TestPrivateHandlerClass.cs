...
internal sealed class TestPrivateHandlerClass(ICalm engine, ILogger logger) : ITestClass
{
    /// <summary>
    /// The Calm engine.
    /// </summary>
    ...
    [CalmHandler]
    private async Task HandleTestCommandAsync(TestCommand command, CancellationToken token)
    {
        var id = Guid.NewGuid();
        try
        {
    ...
    /// <returns>A task representing the response from the handler.</returns>
    [CalmHandler]
    private async Task<TestResponse> HandleTestCommandWithResponseAsync(
    ...
    /// <returns>A task representing the response from the handler.</returns>
    [CalmHandler]
    private async Task<TestResponse> HandleTestQueryAsync(TestQuery query, CancellationToken token)
    {
        var id = Guid.NewGuid();
        TestResponse? response = null;
        try
    ...
    /// <returns>A task representing the response from the handler.</returns>
    [CalmHandler]
    private async Task HandleTestEventAsync(TestEvent @event, CancellationToken token)
    {
        var id = Guid.NewGuid();
        try
        {
